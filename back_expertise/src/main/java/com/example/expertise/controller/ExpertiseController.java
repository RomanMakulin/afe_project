...
public class ExpertiseController {
    ...
    public ResponseEntity<ExpertiseResponseDto> createExpertise(@RequestBody @Valid CreateExpertiseDto createExpertiseDto) {
        return ResponseEntity.ok(new ExpertiseResponseDto(expertiseService.createNewExpertise(createExpertiseDto)));
    }
    ...
    public ResponseEntity<ExpertiseResponseDto> getExpertise(@PathVariable UUID expertiseId) {
        return ResponseEntity.ok(new ExpertiseResponseDto(expertiseService.getExpertiseById(expertiseId)));
    }
    ...
    public ResponseEntity<List<ExpertiseResponseDto>> getExpertisesByProfileId(@PathVariable UUID profileId) {
        List<Expertise> expertises = expertiseService.getExpertiseByProfileId(profileId);
        List<ExpertiseResponseDto> expertiseListDto = expertises.stream()
                .map(ExpertiseResponseDto::new)
                .collect(Collectors.toList());
        return ResponseEntity.ok(expertiseListDto);
    }
