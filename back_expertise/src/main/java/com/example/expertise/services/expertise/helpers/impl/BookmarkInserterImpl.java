...
import static com.example.expertise.util.docs.DocumentProcessor.LINE_SEPARATOR;
import static com.example.expertise.util.docs.DocumentProcessor.QUESTION_SEPARATOR;
...
public class BookmarkInserterImpl implements BookmarkInserter {
    ...
    private String formatQuestionAnswer(ExpertiseQuestion question, int questionNumber) {
        ...
        if (question.getChecklistInstances() != null && !question.getChecklistInstances().isEmpty()) {
            // Группируем по Checklist.id, чтобы избежать дублирования плейсхолдеров
            Set<UUID> checklistIds = question.getChecklistInstances().stream()
                    .map(ci -> ci.getChecklist().getId())
                    .collect(Collectors.toSet());
            for (UUID checklistId : checklistIds) {
                result.append(String.format("%s[CHECKLIST_%s]", LINE_SEPARATOR, checklistId));
            }
        }
